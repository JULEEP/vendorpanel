...
const initialNotices = [
  { id: 1, type: "bhubh", description: "bgvjhgc", date: "2025-01-31", noticeBy: "12" },
  { id: 2, type: "Policy", description: "dededed", date: "2025-01-16", noticeBy: "admin" },
  { id: 3, type: "dfgds", description: "fasdf", date: "2025-01-23", noticeBy: "addd" },
  { id: 4, type: "sample", description: "sample", date: "2024-10-13", noticeBy: "Y" },
  { id: 5, type: "Immediate management meeting", description: "management decision", date: "2024-07-11", noticeBy: "Mr Michael" },
...
export default function NoticeList() {
  ...
  return (
    <div className="p-4 border rounded-lg shadow-lg bg-white">
      <div className="flex justify-between items-center mb-4">
        ...
        <button className="bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 rounded" onClick={() => setShowModal(true)}>
          + Add New Notice
        </button>
      </div>
            ...
            {currentNotices.map((notice, index) => (
              <tr key={notice.id} className="border-b">
                ...
                <td className="p-2 border flex gap-2">
                  <button className="bg-blue-500 text-white p-1 rounded" onClick={() => { setEditModal(true); setSelectedNotice(notice); setFormData(notice); }}>
                    <FaEdit />
                  </button>
                  <button className="bg-red-500 text-white p-1 rounded" onClick={() => { setDeleteModal(true); setSelectedNotice(notice); }}>
                    <FaTrash />
                  </button>
                </td>
              </tr>
  ...
  <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center p-4">
    <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-md sm:max-w-lg md:max-w-xl">
      ...
      {/* Notice Success Modal */}
      {successModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
          <div className="bg-white p-6 rounded-lg shadow-lg w-1/3">
            <h2 className="text-lg font-semibold mb-4">Notice has been successfully updated!</h2>
            <div className="flex justify-end gap-2 mt-4">
              <button className="bg-blue-500 text-white px-4 py-2 rounded" onClick={() => setSuccessModal(false)}>Close</button>
            </div>
          </div>
        </div>
      )}
